...
0. Загрузка данных:
Предполагается, что данные уже загружены в DataFrame df_q3_q4_2024.
Создается копия DataFrame, чтобы избежать изменений в оригинальном DataFrame.
1. Подготовка данных:
Заполняет пропущенные значения в столбце lemmatized_message (вероятно, содержащем лемматизированный текст сообщений) пустой строкой.
...
3. Выбор количества компонент для SVD:
...
Отображает несколько случайных сообщений из кластера для более детального анализа.
 ...
 Получаем такой список кластеров для анализа
